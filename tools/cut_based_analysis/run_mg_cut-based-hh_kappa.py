...
DELPHES_CARD_PATH = "/home/lxiao/Tagging/inputs/delphes_card.dat"
...
def modify_param_card(file_path, kappa_value):
    with open(file_path, 'r', encoding='utf-8') as file:
        data = file.readlines()

    # Locate the line containing "# k3"
    for i, line in enumerate(data):
        if "# k3" in line:
            # Replace the entire line with the new content
            data[i] = f"    1 {kappa_value} # k3\n"
            break
...
def run_process(process, output_dir, process_name, n, root_file_dir, madspin, madspin_card_path, kappa_value):
    try:
    ...
    # 如果 madspin 为 1，则先导入模型
    if int(madspin) == 1:
        subprocess.run(["./bin/mg5_aMC"], input=f"import model SM_k3_EFF_UFO\ngenerate {process}\noutput {process_dir}\nexit\n", text=True)
    else:
    ...
    # 修改param_card.dat文件
    param_card_path = f"{process_dir}/Cards/param_card.dat"
    modify_param_card(param_card_path, kappa_value)

    # 修改run_card.dat文件
    with open(run_card_path, 'r', encoding='utf-8') as file:
        data = file.readlines()

    print(f"Original run_card.dat content:\n{''.join(data)}")  # 调试输出，检查文件内容
    ...
    print(f"Modified run_card.dat content:\n{''.join(data)}")  # 调试输出，检查修改后的文件内容
...
def main(config_file):
    ...
    for line in lines:
        ...
        if len(args) == 8:  # Ensure the length matches the expected number of arguments
            process, output_dir, process_name, n, root_file_dir, madspin, madspin_card_path, kappa_value = args
            processes.append((process, output_dir, process_name, n, root_file_dir, madspin, madspin_card_path, kappa_value))
